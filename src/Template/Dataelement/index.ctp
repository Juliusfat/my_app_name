<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Dataelement[]|\Cake\Collection\CollectionInterface $dataelement
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Dataelement'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Data'), ['controller' => 'Data', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Data'), ['controller' => 'Data', 'action' => 'add']) ?></li>
    </ul>
</nav>
<div class="dataelement index large-9 medium-8 columns content">
    <h3><?= __('Dataelement') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('data_id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('name') ?></th>
                <th scope="col"><?= $this->Paginator->sort('created') ?></th>
                <th scope="col"><?= $this->Paginator->sort('modified') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($dataelement as $dataelement): ?>
            <tr>
                <td><?= $this->Number->format($dataelement->id) ?></td>
                <td><?= $dataelement->has('data') ? $this->Html->link($dataelement->data->name, ['controller' => 'Data', 'action' => 'view', $dataelement->data->id]) : '' ?></td>
                <td><?= h($dataelement->name) ?></td>
                <td><?= h($dataelement->created) ?></td>
                <td><?= h($dataelement->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $dataelement->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $dataelement->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $dataelement->id], ['confirm' => __('Are you sure you want to delete # {0}?', $dataelement->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(['format' => __('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')]) ?></p>
    </div>
</div>
